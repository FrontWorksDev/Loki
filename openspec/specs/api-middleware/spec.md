...
#### Scenario: 上限ちょうどの raw body
- **WHEN** クライアントが `Content-Length: 33554432` (32 MiB ぴったり) の raw body で `POST /api/v1/compress` を送信する
- **THEN** リクエストはハンドラに到達する (BodyLimit ミドルウェアは `ContentLength <= max_bytes` の境界では拒否しない)

#### Scenario: multipart upload の実用上限 (multipart overhead 考慮)
- **WHEN** クライアントが `multipart/form-data` 形式で 32 MiB ぴったりのファイルをアップロードする
- **THEN** multipart 境界・ヘッダ・フィールド名等の数百バイトの overhead により実際の `Content-Length` が 32 MiB を超え、413 が返る。multipart 利用時の実用上限は 31 MiB 程度
