...
APIサーバーは受信リクエストのボディサイズに対して上限を強制しなければならない（MUST）。デフォルトは 50 MiB (52,428,800 バイト) とし、設定 `api.body_limit_bytes` で上書き可能でなければならない（MUST）。上限超過時は HTTP 413 (Payload Too Large) を返し、ハンドラには到達させてはならない（MUST NOT）。

#### Scenario: 上限以内のリクエスト
- **WHEN** クライアントが49MiBのボディで `POST /api/v1/compress` を送信する
- **THEN** リクエストはハンドラに到達し、通常通り処理される

#### Scenario: 上限超過リクエスト
- **WHEN** クライアントが51MiBのボディで `POST /api/v1/compress` を送信する
...
CORS、ロギングレベル、ボディサイズ上限、レートリミットの各設定値は `configs/default.yaml` の `api:` セクションおよび環境変数（プレフィックス `LOKI_API_`、ネストはアンダースコアで表現）から読み込み可能でなければならない（MUST）。設定ファイル不在時は安全側のデフォルト（オリジン `["*"]`、ボディ50MB、30req/分・バースト10、ログレベル `info`）が適用されなければならない（MUST）。
